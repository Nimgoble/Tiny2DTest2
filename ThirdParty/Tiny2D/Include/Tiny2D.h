...
namespace Tiny2D
{
	...
	public:
		...
		//! Draws texture centered
		void DrawCentered(const Vec2& center, float rotation = 0.0f, float scale = 1.0f, const Color& color = Color::White);
	...
	public:
		...
		struct DrawParams
		{
			...
			bool centered;		//!< Draw the sprite centered at given coordinates; defaults to false
		...
		bool Create(const std::string& name, bool immediate = true, AtlasType atlasType = AtlasType::AtlasType_XNA);
		//! Destroys the sprite
		void Destroy();
	...
	public:
		...
		struct StartupParams : DisplaySettings
		{
			...
			bool disableCursor;				//!< Exactly what it sounds like.
